...
class InvalidMoneyType(Exception):
    ...
    Args:
        Exception (InvalidMoneyType): Object type is not compliant to Money type
    """
    def __init__(self, msg: str = "Object type not compliant with Money type") -> None:
        """Exception initializer        

        Args:
            msg (str, optional): Error when object is not from Money type. Defaults to "Object type not compliant with Money type".
        """
        self.message = msg
        super().__init__(self.message)


def count_digits(num: float |Decimal) -> int:
    ...
    if isinstance(num, float):
        ...
        raise ValueError("Number must be in string, float or int format")
...
def count_decimals(num: float |Decimal):
    """Counts the total number of decimals in a given floating-point or Decimal number.
...
class Money:
    ...
    def __post_init__(self) -> None:
        """Initialize the Money instance.

        This method is called after the instance has been initialized.
        It sets the number of digits in the `amount_cents` attribute.
        """
        self.digits = count_digits(self.amount_cents)

    @classmethod
    def mint(cls, amount: Decimal | float, currency_symbol: Symbol = Symbol("EUR")) -> 'Money':
        """Create a new Money instance.

        This method creates a new Money instance using the given `amount` and `currency_symbol`.
        The `precision` attribute is determined based on the number of decimals in the `amount`.

        Args:
            amount (Decimal or float): The value of the money.
            currency_symbol (Symbol, optional): The symbol of the currency. Defaults to "EUR".
    ...
    def __str__(self) -> str:
        ...
        """
        return f"{self.currency_symbol}{self.amount_cents / 100:.2f}"

    def __add__(self, other: Self) -> Self:
        """Add two Money instances.

        This method adds two Money instances together if they have the same currency symbol.
        ...
        Raises:
            InvalidMoneyType: If the `other` parameter is not an instance of the Money class.
        """
        if isinstance(other, Money) and self.currency_symbol == other.currency_symbol:
            return Money(self.amount_cents + other.amount_cents, self.precision, self.currency_symbol)
        else:
    ...
    def __sub__(self, other: Self) -> Self:
        ...
        This method subtracts the `other` Money instance from the current Money instance.
