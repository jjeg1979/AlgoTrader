...
from src.backtests.htmlparser import (
    ...
    get_balance_from_ops,  # type: ignore
    get_mt4_operations,
    get_gbx_operations,
...
payload: list[str] = [file for file in payload
...
class TestHTMLParser:
    ...
    @pytest.fixture
    def result(self) -> str:
        result: str = read_html_file(
            Path(PAYLOAD_DIR) / payload[0])  # type: ignore
        return result
    ...
    def test_extract_header_information(self):
        bt: list[pd.DataFrame] = process_backtest(
            Path(PAYLOAD_DIR) / payload[1])
    ...
    def test_extract_mt4_operations_from_df(self):
        ...
        expected_column_names: list[str] = list(
            COLUMNS_FOR_MT4_FROM_HTML.values())[1:]
    ...
    def test_extract_gbx_operations_from_df(self):
        ...
        assert isinstance(ops, pd.DataFrame)
        expected_column_names: list[str] = list(
            COLUMNS_FOR_GBX_FROM_HTML.values())[1:]
        assert ops.columns.to_list() == expected_column_names  # type: ignore
        assert ops.index.name == "#"  # type: ignore

    def test_transform_mt4_to_gbx(self):
        bt: list[pd.DataFrame] = process_backtest(Path(
            PAYLOAD_DIR) / payload[1])
        ops: pd.DataFrame = extract_mt4_operations_information(bt[1])
    ...
    def test_get_balance_from_ops(self):
        ...
        mt4_balance: pd.Series = get_balance_from_ops(  # type: ignore
            mt4_ops["Profit"], initial_bal  # type: ignore
        )  # type: ignore
        gbx_balance: pd.Series = get_balance_from_ops(  # type: ignore
            gbx_ops["Profit"], initial_bal  # type: ignore
        )  # type: ignore

        assert (mt4_balance.iloc[-1] - initial_bal) == mt4_ops[  # type: ignore
            "Profit"
        ].sum()  # type: ignore
        assert (gbx_balance.iloc[-1] - initial_bal) == gbx_ops[  # type: ignore
            "Profit"
        ].sum()  # type: ignore
